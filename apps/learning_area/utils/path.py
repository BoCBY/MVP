...
def create_folder(lecturer, course, video_id):
    ...
    exists_entry = os.listdir(LEARNING_COMPUTER_PATH)
    for name in exists_entry:
        # 檢查看看此影片是否已經被用戶看過, 若有則不做任何事
        if video_id in name:
            # print('該url已看過')
            exists_folder = os.path.join(LEARNING_COMPUTER_PATH, name)
            return exists_folder # 要進入這個已經存在的資料夾, 所以要返回這個路徑
    
    # 如果此url影片沒有被用戶看過, 則創建一個資料夾
    folder_name = lecturer + '[]' + course + '[]' + video_id
    new_folder = os.path.join(LEARNING_COMPUTER_PATH, folder_name) # 此新創的資料夾的路徑
...
def find_path(partial_name, start_path=LEARNING_COMPUTER_PATH):
